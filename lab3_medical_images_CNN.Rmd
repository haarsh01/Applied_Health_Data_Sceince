...
Pneumonia is an infection in the lungs that causes the air sacs to become inflamed and often filled with fluid. Symptoms include coughing, fever, chills, difficulty breathing, chest pain, and fatigue. The severity can vary from mild to severe, posing significant risks to vulnerable groups .

And the point of being able to identify it from X-rays automatically is that automated systems can help reduce mistakes that happen when radiologists are tired or overlook something. These systems use algorithms that consistently check X-rays, making it easier to accurately detect pneumonia.Its accurate and scalable. 
...
The filenames of the X-ray images offer basic details, mainly indicating whether the condition is NORMAL or PNEUMONIA. For pneumonia images, they also specify whether the cause is bacterial or viral.

The challenges in predicting Pneumonia can be the filenames lack details about pneumonia severity, specific pathogens, patient age, or other conditions, limiting diagnostic context. Additionally, they don't indicate image quality or exposure, which can affect automated prediction consistency.
...
From reviewing these images, it's clear that certain attributes need to be normalized before training a model. And firstly is to verify that all images have a consistent orientation. For example, ensure the "R" marker is always positioned on the right side throughout the entire dataset.Also the images need to be resized to a uniform resolution, as varying image sizes can impact the performance of convolutional neural networks.
...
If the dataset was not balanced then the oversampling tecnique will be useful to  enhance the representation of the minority class by increasing its instances. And in Random Oversampling it just randomly create duplicates of the minority class instances.

Secondly, data augmentation particularly effective in areas like image processing, where methods like flipping and scaling images can produce more instances for the minority class.
...
One drawback can be overfitting If the augmented data lacks sufficient diversity, the model may overfit to these synthetic variations, particularly when the original dataset is small. Secondly, Certain augmentation methods are specific to particular domains and may not be applicable to other types of data. For instance, techniques that work well for image data might not be effective for text or tabular data. And if the original dataset contains biases, data augmentation might replicate and potentially amplify these biases instead of reducing them.
...
ImageNet is a vast dataset of labeled images aimed at advancing algorithms for recognizing and categorizing objects in visual data. It serves as a critical resource for developing and benchmarking computer vision models.

ImageNet comprised more than 14 million annotated images spanning over 20,000 distinct categories or classes.
 
The extensive variety of images and categories in ImageNet has enabled effective training of deep learning models. Pre-trained models from ImageNet are commonly used for transfer learning, making it easier to apply them to new tasks with limited labeled data.
...
Employing the MobileNet architecture in practical tasks can enhance outcomes significantly, particularly in scenarios involving transfer learning.
Transfer learning with MobileNet involves utilizing its pre-training on extensive datasets like ImageNet to capture broad image features applicable across various tasks. By fine-tuning these pre-learned features on a smaller dataset specific to the practical assignment, we harness MobileNet's rich feature representations. And MobileNet's architecture is flexible enough to be used for various computer vision tasks beyond just identifying objects in images. It can also handle tasks like detecting objects and segmenting images.


**9_** How many parameters does this network have? How does this compare to better performing networks available in torch?

This network has 4.2 Million parameters. This compare to better performing networks available in torch because VGG 16 has 138 Million parameters. 
...
#fmobilenet<-model_mobilenet_v2(pretrained = TRUE)
